...
function vpb_multiple_file_uploader(vpb_configuration_settings)
{
	...
	vpb_multiple_file_uploader.prototype.vpb_show_added_files = function(vpb_value)
	{
		...
		$('.progress-bar').attr('aria-valuemax', this.vpb_files.length);
		...
		if(this.vpb_files.length > 0)
		{
 			...
 			for(var i = 0; i<this.vpb_files.length; i++)
			{
				...
				if(typeof this.vpb_files[i] != undefined && this.vpb_files[i].name != "")
				{
					...
					vpb_added_files_displayer += '<tr id="add_fileID'+vpb_file_id+'" class="'+new_classc+'"><td id="number">'+(i+1)+'</td><td>'+vpb_file_icon+' '+this.vpb_files[i].name.substring(0, 40)+'</td><td><span id="uploading_'+vpb_file_id+'"><span style=color:blue;>Ready</span></span></td><td>'+vpb_actual_fileSize+'</td><td><span id="remove'+vpb_file_id+'"><span class="vpb_files_remove_left_inner" onclick="vpb_remove_this_file(\''+vpb_file_id+'\',\''+this.vpb_files[i].name+'\');">Remove</span></span></td></tr></div>';
					...
					success:function(response)
					{
						setTimeout(function() {
							...
							if ( response_brought != -1) {
								var uploaded = $('#number').html();
								var total = $('.progress-bar').attr('aria-valuemax');
								$('.progress-bar').attr('aria-valuenow', uploaded);
								$('.progress-bar').attr('style', 'width:'+Math.round(((uploaded/total)*100))+'%');
								$('.progress-bar').text(Math.round(((uploaded/total)*100))+'%');
								...
								$("#uploading_"+ids).parent().parent().remove();
							} else {
